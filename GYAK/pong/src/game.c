...
void update_game(Game* game, double time)
{
    ...
    if (game->ball.x - game->ball.radius < 50 && game->left_pad.position <= game->ball.y && game->left_pad.position+game->left_pad.size >= game->ball.y) {
        game->ball.x = game->ball.radius + 50;
        game->ball.speed_x *= -1;
    }
    if (game->ball.x + game->ball.radius > game->width - 50 && game->right_pad.position <= game->ball.y && game->right_pad.position+game->right_pad.size >= game->ball.y) {
...
void drop_ball(Game* game, int x, int y)
{
	move_ball(&(game->ball), x, y);
}

void resize_ball(Game* game, int amount)
{
	rescale_ball(&(game->ball), amount);
}